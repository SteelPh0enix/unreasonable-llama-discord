    ...
    async def get_buffered_llm_response(self, prompt: str, message_length: int) -> AsyncIterator[LlamaResponseChunk]:
        ...
        async for chunk in self.llama.get_streamed_completion(request):
            ...
            if next_message is None:
                yield LlamaResponseChunk(message, chunk.content, response, False, chunk.stop)
            else:
                yield LlamaResponseChunk(current_message, None, response, False, False)
                yield LlamaResponseChunk(next_message, chunk.content, response, True, chunk.stop)
